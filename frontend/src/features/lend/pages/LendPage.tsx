...
import { isFiatSupportedChain, getTokenByAddressForChain } from '@/config/contracts';
import { useNetwork } from '@/contexts/NetworkContext';
import { useCryptoMarketplaceRequests, useFiatMarketplaceRequests } from '@/hooks/useMarketplaceListings';
...
export default function LendPage() {
  ...
  const { data: cryptoRequests } = useCryptoMarketplaceRequests(selectedNetwork.id);

  const fiatSupported = isFiatSupportedChain(selectedNetwork.id);
  const { data: fiatRequests } = useFiatMarketplaceRequests();

  // Calculate total available borrow requests (excluding user's own and invalid requests)
  const totalAvailableRequests = useMemo(() => {
    const cryptoCount = cryptoRequests.filter((request) => {
      if (isConnected && address && request.borrower.toLowerCase() === address.toLowerCase()) return false;
      return !!getTokenByAddressForChain(request.collateralToken as `0x${string}`, selectedNetwork.id)
        && !!getTokenByAddressForChain(request.borrowAsset as `0x${string}`, selectedNetwork.id);
    }).length;

    const fiatCount = fiatSupported
      ? fiatRequests.filter((request) => !(isConnected && address && request.borrower.toLowerCase() === address.toLowerCase())).length
      : 0;

    return cryptoCount + fiatCount;
  }, [isConnected, address, cryptoRequests, fiatRequests, fiatSupported, selectedNetwork.id]);
