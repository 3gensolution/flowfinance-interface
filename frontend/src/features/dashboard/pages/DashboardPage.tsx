...
import { useRefreshContractData } from '@/hooks/useRefreshContractData';
import {
  SummaryCards,
  FiatSupplierPanel,
  ActiveLoansOffers,
...
export default function DashboardPage() {
  ...
  const { isRefreshing, refresh } = useRefreshContractData();
    ...
    <div className="min-h-screen py-12 px-4 sm:px-6 lg:px-8">
      <div className="max-w-[1920px] mx-auto px-4 lg:px-8">
        ...
        >
          ...
          <Button
            variant="secondary"
            icon={isRefreshing ? <Loader2 className="w-4 h-4 animate-spin" /> : <RefreshCw className="w-4 h-4" />}
            onClick={() => refresh()}
