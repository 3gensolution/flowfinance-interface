...
import { TOKEN_LIST, isFiatSupportedChain, getTokenListForChain, getChainById } from '@/config/contracts';
import { useNetwork, NETWORKS_WITH_CONTRACTS } from '@/contexts/NetworkContext';
import { useBorrowPageState } from '@/features/borrow/hooks/useBorrowPageState';
import ERC20ABIJson from '@/contracts/ERC20ABI.json';
import { useCryptoMarketplaceOffers, useFiatMarketplaceOffers } from '@/hooks/useMarketplaceListings';
...
export default function BorrowPage() {
  ...
  const { data: cryptoOffers } = useCryptoMarketplaceOffers(selectedNetwork.id);

  const fiatSupported = isFiatSupportedChain(selectedNetwork.id);
  const { data: fiatOffers } = useFiatMarketplaceOffers();

  // Calculate total available offers (excluding user's own offers)
  const totalAvailableOffers = useMemo(() => {
    if (!isConnected || !address) {
      return cryptoOffers.length + (fiatSupported ? fiatOffers.length : 0);
    }
    const cryptoCount = cryptoOffers.filter((offer) => offer.lender.toLowerCase() !== address.toLowerCase()).length;
    const fiatCount = fiatSupported
      ? fiatOffers.filter((offer) => offer.lender.toLowerCase() !== address.toLowerCase()).length
      : 0;
    return cryptoCount + fiatCount;
  }, [isConnected, address, cryptoOffers, fiatOffers, fiatSupported]);
  ...
  const { state, actions } = useBorrowPageState({
  ...
  const stablecoins: StablecoinAsset[] = useMemo(() => {
    ...
    return tokenSource
      .filter(token => STABLECOIN_SYMBOLS.includes(token.symbol))
      .map(token => ({
        symbol: token.symbol,
        name: token.name,
        address: token.address,
        ...
        return (
          <div className="space-y-6">
            ...
            <CollateralSelector
              ...
              assets={collateralAssets.filter(a => !STABLECOIN_SYMBOLS.includes(a.symbol))}
