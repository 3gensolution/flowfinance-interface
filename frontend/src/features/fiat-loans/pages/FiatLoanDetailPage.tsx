...
export default function FiatLoanDetailPage() {
  ...
  const loan = loanData ? {
    ...
    borrower: (rawLoan.borrower ?? rawLoan[2]) as Address,
    supplier: (rawLoan.supplier ?? rawLoan[3]) as Address,
    collateralAsset: (rawLoan.collateralAsset ?? rawLoan[4]) as Address,
    collateralAmount: (rawLoan.collateralAmount ?? rawLoan[5]) as bigint,
    fiatAmountCents: (rawLoan.fiatAmountCents ?? rawLoan[6]) as bigint,
    currency: (rawLoan.currency ?? rawLoan[7]) as string,
    interestRate: (rawLoan.interestRate ?? rawLoan[8]) as bigint,
    duration: (rawLoan.duration ?? rawLoan[9]) as bigint,
    status: Number(rawLoan.status ?? rawLoan[10]) as FiatLoanStatus,
    createdAt: (rawLoan.createdAt ?? rawLoan[11]) as bigint,
    activatedAt: (rawLoan.activatedAt ?? rawLoan[12]) as bigint,
    dueDate: (rawLoan.dueDate ?? rawLoan[13]) as bigint,
    gracePeriodEnd: (rawLoan.gracePeriodEnd ?? rawLoan[14]) as bigint,
    claimableAmountCents: (rawLoan.claimableAmountCents ?? rawLoan[15]) as bigint,
    fundsWithdrawn: (rawLoan.fundsWithdrawn ?? rawLoan[16]) as boolean,
    repaymentDepositId: (rawLoan.repaymentDepositId ?? rawLoan[17]) as `0x${string}`,
    exchangeRateAtCreation: ((rawLoan.exchangeRateAtCreation ?? rawLoan[18]) as bigint) || BigInt(0),
    chainId: ((rawLoan.chainId ?? rawLoan[19]) as bigint) || BigInt(0),
  } : null;
